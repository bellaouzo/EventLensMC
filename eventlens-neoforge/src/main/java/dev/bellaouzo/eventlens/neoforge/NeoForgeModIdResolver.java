...
final class NeoForgeModIdResolver implements AgentRuntime.OwnerIdResolver {

    private final Map<String, String> classOwners = new ConcurrentHashMap<>();

    @Override
    public String resolve(Object listener) {
        Object target = firstPresent(listener, "instance", "consumer");
    ...
    private static String lookup(Class<?> type) {
        try {
            ...
            for (var fileInfo : ModList.get().getModFiles()) {
                ...
                boolean match = scan.getClasses().stream().anyMatch(data -> ownsClass(data.clazz().getClassName(), className));
                if (match) {
                    return modId;
                }
            }
