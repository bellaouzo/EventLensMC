...
- NeoForge client mod no longer crashes on startup when registering the `eventlens:correlate` payload (use `playBidirectional` instead of duplicate `playToClient` / `playToServer` registration on 26.2)
