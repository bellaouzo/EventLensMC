    ...
    <section class="page">
      <header class="page-header">
        ...
        <p class="page-subtitle mono">
          ${v(A(s.eventClassName))}${d?` · ${v(d)}`:""}${s.peerSessionId?` · linked ${v(s.peerSessionId)}`:""}${s.serverTick!=null?` · server tick ${s.serverTick}`:""}${s.msptMillis!=null?` · ${s.msptMillis.toFixed(1)} mspt`:""} · #${s.sequence} · total <span class="severity-${c}">${N(s.durationNanos)} (${o.toFixed(1)}% of tick budget)</span>
        </p>
      </header>
      ${n}
