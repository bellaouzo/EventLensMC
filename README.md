...
|---|---|---|
| **Paper plugin** `EventLens-1.2.4.jar` | Server operators and plugin authors | Yes, for server traces |
| **Paper Java agent** `eventlens-agent-1.2.4.jar` | Per-listener timing and exception attribution | Optional |
| **Client mods** NeoForge / Forge / Fabric | Client-side traces, Screen, HUD | Optional |
| **Client Java agent** `eventlens-client-agent-1.2.4.jar` | Per-mod handler timing on NeoForge and Forge | Optional; not on Fabric yet |
...
2. Drop `eventlens-paper/build/libs/EventLens-1.2.4.jar` into `plugins/`.
...
```
eventlens-paper/build/libs/EventLens-1.2.4.jar
```
...
```
-javaagent:eventlens-agent-1.2.4.jar
```
...
|---|---|---|
| **NeoForge** 21.1.x | 1.21.1 | `eventlens-neoforge/build/libs/eventlens-neoforge-1.2.4.jar` |
| **Minecraft Forge** 52.1.x | 1.21.1 | `eventlens-forge/build/libs/eventlens-forge-1.2.4.jar` |
| **Fabric** Loader 0.16 + Fabric API | 1.21.1 | `eventlens-fabric/build/libs/eventlens-fabric-1.2.4.jar` |
...
```
-javaagent:eventlens-client-agent-1.2.4.jar
```

Place `eventlens-observability-1.2.4.jar` in the **same folder** as the agent jar. Dev tasks `:eventlens-neoforge:runClient` and `:eventlens-forge:runClient` attach it automatically. Fabric `runClient` does not — Fabric stays `dispatch-only` until a stable invoker exists.
...
Views: **Overview**, **Timeline**, **Flame graph**, **Event graph**, **Plugin graph**, **Compare**.
